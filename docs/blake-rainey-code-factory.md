---
summary: "Blake Rainey's Code Factory pattern — the blueprint our CI/review pipeline implements"
read_when:
  - Setting up or modifying the CI pipeline
  - Working on code review automation
  - Adding new risk tiers or merge policies
  - Understanding the review agent architecture
---
...
On this repo, the concrete implementation target is GitHub PRs + GitHub Actions checks + `gh` automation.

The specific review agent can be Greptile, CodeRabbit, CodeQL + policy logic, custom LLM review, or another service. The control-plane pattern stays the same.

Inspired by industry patterns and @_lopopolo

---

"I've been grinding with Codex (on Extra High) through setting up our repo for Harness Engineering. The goal is to have Codex write and review 100% of the code. Getting much closer."
...
GitHub-oriented equivalents for this repo

- Open/update PRs with `gh pr create`, `gh pr view`, and `gh pr checks`
- Fetch PR metadata and review state with `gh api repos/<owner>/<repo>/pulls/<number>`
- Treat GitHub check runs as the authoritative merge gate
