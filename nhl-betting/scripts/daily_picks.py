...
players_15 = set(p['player'] for p in all_props if p['line'] == 1.5 and p['side'] == 'Over' and p['market'] == 'player_points')
players_05 = set(p['player'] for p in all_props if p['line'] == 0.5 and p['side'] == 'Over' and p['market'] == 'player_points')
...
for p in c_picks:
    all_picks.append({
        ...
        'reasoning': f"{p['mp_games']}/{p['gp']} multi-pt games ({p['mp_rate']*100:.1f}%), {p['ppg']:.2f} P/GP, BE={p['breakeven']*100:.1f}%"
    })
...
for p in b2_picks[:15]:  # Top 15 singles
    all_picks.append({
        ...
        'reasoning': f"{p['point_games']}/{p['gp']} point games ({p['point_rate']*100:.1f}%), {p['ppg']:.2f} P/GP, BE={p['breakeven']*100:.1f}%"
    })
